...
alter table public.freights
  ...
  add column if not exists pod_received_by uuid references public.profiles(id);

update public.freights
set
  pod_received_date = coalesce(pod_received_date, ack_received_at::date),
  pod_file_path = coalesce(
    pod_file_path,
    nullif(delivery_stages #>> '{delivered,pod_photo_path}'::text[], '')
  )
where ack_status = 'received'
  or nullif(delivery_stages #>> '{delivered,pod_photo_path}'::text[], '') is not null;
